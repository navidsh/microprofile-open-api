...
import org.eclipse.microprofile.openapi.tck.parser.core.models.ParseOptions;
import org.eclipse.microprofile.openapi.tck.parser.core.models.SwaggerParseResult;
import org.eclipse.microprofile.openapi.tck.parser.util.ClasspathHelper;
import org.eclipse.microprofile.openapi.tck.parser.util.InlineModelResolver;
import org.eclipse.microprofile.openapi.tck.parser.util.OpenAPIDeserializer;
import org.eclipse.microprofile.openapi.tck.parser.util.RemoteUrl;
import org.eclipse.microprofile.openapi.tck.parser.util.ResolverFully;
...
public class OpenAPIV3Parser {
    ...
    public OpenAPI read(String location, List<AuthorizationValue> auths, ParseOptions resolve) {
        ...
        return new OpenAPIV3Parser().readLocation(location, auths, resolve).getOpenAPI();
    }
    ...
    public SwaggerParseResult readWithInfo(String location, List<AuthorizationValue> auths) {
        ...
        try {
            ...
            output.setMessages(Arrays.asList("unable to read location `" + location + "` due to a SSL configuration error.  "
                    + "It is possible that the server SSL certificate is invalid, self-signed, or has an untrusted " + "Certificate Authority."));
            return output;
        } catch (Exception e) {
            SwaggerParseResult output = new SwaggerParseResult();
    ...
    /**
     * Transform the swagger-model version of AuthorizationValue into a
     * parser-specific one, to avoid dependencies across extensions
     *
     * @param input
     * @return
     */
